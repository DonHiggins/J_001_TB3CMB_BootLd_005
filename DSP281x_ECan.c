...
// HECC mode also enables time-stamping feature
	...
	ECanaShadow.CANMC.bit.SCB = 1; // 1 = eCAN mode (new style, "enhanced"), 0 = SCC mode (old style, "compatible")
    // apply lsb-first byte orderring to CAN messages -- DH added 11/20/2014
	// this gets DSP to receive data in reasonable byte order, not byte-reversed
	ECanaShadow.CANMC.bit.DBO = 1;  //data is received or transmitted least significant byte first
...
/* Configure bit timing parameters */
	...
	ECanaShadow.CANMC.all = ECanaRegs.CANMC.all;
	ECanaShadow.CANMC.bit.CCR = 1 ;            		// Set CCR = 1, then wait for permission . . .
    ECanaRegs.CANMC.all = ECanaShadow.CANMC.all;
    ...
    ECanaShadow.CANBTC.bit.BRPREG = 14;				// 1 Mbps @ 150 MHz SYSCLKOUT
    ECanaShadow.CANBTC.bit.TSEG2REG = 2;            // BRPREG, TSEG2REG & TSEG1REG set to emulate DSP2407
    ECanaShadow.CANBTC.bit.TSEG1REG = 5;            // -- DH 11/15/2013
